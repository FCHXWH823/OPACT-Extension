...
int NTEST; //default number of tests for simulation
...
int main()
{
	...
	NTEST = (2 * MULT_SIZE > 20) ? (1 << 20) : (1 << (2 * MULT_SIZE));
	...
	for (auto bound : bounds)
	{
		...
		try {
			...
			{
				obj += (5.05 * fa_num + 2.66 * ha_num + 2.66 * ac32_num + 4.78 * ac42_num) + Weight * E;
			}
			...
			cout<< "Area" << (5.05 * nFA + 2.66 * nHA + 2.66 * nAC32 + 4.78 * nAC42) << endl;
			Opt1_Analyzed_Error = E.get(GRB_DoubleAttr_X);
			/*repeatly*/
			
			for (int i = 0; i < variables_V[variables_V.size() - 1].size(); i++)
			{
				cout << unsigned(variables_V[variables_V.size() - 1][i].get(GRB_DoubleAttr_X)) << ", ";
				last_stage.push_back(unsigned(variables_V[variables_V.size() - 1][i].get(GRB_DoubleAttr_X)));
			}

			max_num_bits = *max_element(last_stage.begin(), last_stage.end());
			cout << endl << endl;
		...
		try {
			...
			// vector<vector<int>> last_stage_connection(max_num_bits, vector<int>(2 * MULT_SIZE + stages_num - 1, 0));
			// for (int i = 0; i < connect_last_stage.size(); i++)
			// {
			// 	for (int j = 0; j < connect_last_stage[i].size(); j++)
			// 	{
			// 		if (connect_last_stage[i][j].get(GRB_DoubleAttr_X))
			// 		{
			// 			last_stage_connection[j][i] = 1;
			// 			// move some of the remaining bits of one column to the end of the vector in the last stage
			// 			auto it = std::find(allSols[allSols.size()-1][i].col_iRB_sel.begin(), allSols[allSols.size()-1][i].col_iRB_sel.end(), j);
			// 			if (it != allSols[allSols.size()-1][i].col_iRB_sel.end())
			// 				std::rotate(it, it + 1, allSols[allSols.size()-1][i].col_iRB_sel.end());
			// 		}
			// 	}
			// }

			// cout << "Last stage connection: " << endl;
			// for (int i = 0; i < last_stage.size(); i++) {
			// 	cout << last_stage[i] << ", ";
			// }
			// cout << endl;
			
			// for (int i = 0; i < last_stage_connection.size(); i++)
			// {
			// 	for (int j = 0; j < last_stage_connection[i].size(); j++)
			// 	{
			// 		cout << last_stage_connection[i][j] << ", ";
			// 	}
			// 	cout << endl;
			// }
			...
			if (!Opt_Mode)
			{
				...
				system(("mkdir -p CEOA/" + to_string(MULT_SIZE) + "/" + to_string(int(MED_bound))).c_str());
			}
			else if (Opt_Mode == 1)
			{
				if (simulator_approach == 0)
				...
				system(("mkdir -p CAOE/" + to_string(MULT_SIZE) + "/" + to_string(AREA_bound)).c_str());
			}
			else
			{
				if (simulator_approach == 0)
				...
				system(("mkdir -p CoOpt/" + to_string(MULT_SIZE) + "/" + to_string(Weight)).c_str());
			}
			ofstream f2;
			...
			else if (Opt_Mode == 1)
			{
				...
				system(("cd CAOE/" + to_string(MULT_SIZE) + "/" + to_string(AREA_bound) + "& python \"" + path + "ApproxMULT_MyHDL.py\" \"" + path + file + "\" ILP_ApproxMult " + to_string(MULT_SIZE) + " " + simulation_path).c_str());
				system(("cd " + simulation_path + "; ./run.sh -w " + to_string(MULT_SIZE) + " -n " + to_string(NTEST) + " -m approx_mult 1>/dev/null").c_str());
				system(("cd " + simulation_path + "; ./sim").c_str());
			...
			else
			{
				...
				system(("cd CoOpt/" + to_string(MULT_SIZE) + "/" + to_string(Weight) + "& python \"" + path + "ApproxMULT_MyHDL.py\" \"" + path + file + "\" ILP_ApproxMult " + to_string(MULT_SIZE) + " " + simulation_path).c_str());
				system(("cd " + simulation_path + "; ./run.sh -w " + to_string(MULT_SIZE) + " -n " + to_string(NTEST) + " -m approx_mult 1>/dev/null").c_str());
				system(("cd " + simulation_path + "; ./sim").c_str());
			}
