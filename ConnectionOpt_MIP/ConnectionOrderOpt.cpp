...
// 				{
// 					vector<GRBVar> probs4_k = { probs4[4 * k], probs4[4 * k + 1], probs4[4 * k + 2], probs4[4 * k + 3] };
// 					stringstream ss; string s;
...
// 					model.addQConstr(Qterm0 == probs4_k[2] * probs4_k[3]);
// 					ss << "q4t" << i << "_" << j << "_" << k << "_" << 1;
// 					ss >> s; ss.clear(); ss.str("");
// 					GRBVar Qterm1 = model.addVar(0, GRB_INFINITY, 0, GRB_CONTINUOUS, s);
// 					model.addQConstr(Qterm1 == probs4_k[0] * probs4_k[3]);
// 					ss << "q4t" << i << "_" << j << "_" << k << "_" << 2;
// 					ss >> s; ss.clear(); ss.str("");
// 					GRBVar Qterm2 = model.addVar(0, GRB_INFINITY, 0, GRB_CONTINUOUS, s);
// 					model.addQConstr(Qterm2 == probs4_k[0] * probs4_k[1]);
// 					ss << "q4t" << i << "_" << j << "_" << k << "_" << 3;
// 					ss >> s; ss.clear(); ss.str("");
// 					GRBVar Qterm3 = model.addVar(0, GRB_INFINITY, 0, GRB_CONTINUOUS, s);
// 					model.addQConstr(Qterm3 == probs4_k[1] * probs4_k[3]);
// 					ss << "q4t" << i << "_" << j << "_" << k << "_" << 4;
// 					ss >> s; ss.clear(); ss.str("");
// 					GRBVar Qterm4 = model.addVar(0, GRB_INFINITY, 0, GRB_CONTINUOUS, s);
// 					model.addQConstr(Qterm4 == probs4_k[1] * probs4_k[2]);

// 					vector<GRBQuadExpr> output_probs;
// 					if (isApprox == 1)
// 						output_probs = Approx_AC42_Output_Probs(probs4_k, { Qterm0,Qterm1,Qterm2,Qterm3,Qterm4 });
// 					else if (isApprox == 2)
// 						output_probs = Approx_AC42_Output_Probs_2(probs4_k, { Qterm0,Qterm1,Qterm2,Qterm3,Qterm4 });
// 					else
// 						output_probs = Exact_AC42_Output_Probs(probs4_k, { Qterm0,Qterm1,Qterm2,Qterm3,Qterm4 });
...
// 				{
// 					vector<GRBVar> probs3_k = { probs3[3 * k], probs3[3 * k + 1], probs3[3 * k + 2] };
// 					stringstream ss; string s;
// 					// ac32's quadratic term: i-th stage, j-th column, k-th ac32, 0-th term
// 					ss << "q3t" << i << "_" << j << "_" << k << "_" << 0;
// 					ss >> s; ss.clear(); ss.str("");
// 					GRBVar Qterm0 = model.addVar(0, GRB_INFINITY, 0, GRB_CONTINUOUS, s);
// 					model.addQConstr(Qterm0 == probs3_k[0] * probs3_k[1]);

// 					vector<GRBQuadExpr> output_probs;
// 					if (isApprox == 1)
// 						output_probs = Approx_AC32_Output_Probs(probs3_k, { Qterm0 });
// 					else if (isApprox == 2)
// 						output_probs = Approx_AC32_Output_Probs_2(probs3_k, { Qterm0 });
// 					else
// 						output_probs = Exact_AC32_Output_Probs(probs3_k, { Qterm0 });
...
// 				{
// 					vector<GRBVar> probsf_k = { probsf[3 * k], probsf[3 * k + 1], probsf[3 * k + 2] };
// 					stringstream ss; string s;
// 					// f's quadratic term: i-th stage, j-th column, k-th f, 0-th term
// 					ss << "qft" << i << "_" << j << "_" << k << "_" << 0;
// 					ss >> s; ss.clear(); ss.str("");
// 					GRBVar Qterm0 = model.addVar(0, GRB_INFINITY, 0, GRB_CONTINUOUS, s);
// 					model.addQConstr(Qterm0 == probsf_k[0] * probsf_k[1]);

// 					ss << "qft" << i << "_" << j << "_" << k << "_" << 1;
// 					ss >> s; ss.clear(); ss.str("");
// 					GRBVar Qterm1 = model.addVar(0, GRB_INFINITY, 0, GRB_CONTINUOUS, s);
// 					model.addQConstr(Qterm1 == probsf_k[0] * probsf_k[2]);

// 					ss << "qft" << i << "_" << j << "_" << k << "_" << 2;
// 					ss >> s; ss.clear(); ss.str("");
// 					GRBVar Qterm2 = model.addVar(0, GRB_INFINITY, 0, GRB_CONTINUOUS, s);
// 					model.addQConstr(Qterm2 == probsf_k[1] * probsf_k[2]);

// 					vector<GRBQuadExpr> output_probs;
// 					if (isApprox)
// 						output_probs = Approx_F_Output_Probs(probsf_k, { Qterm0,Qterm1,Qterm2 });
// 					else
// 						output_probs = Exact_F_Output_Probs(probsf_k, { Qterm0,Qterm1,Qterm2 });
...
// 				{
// 					vector<GRBVar> probsh_k = { probsh[2 * k], probsh[2 * k + 1] };
// 					model.addQConstr(Probs[Starth + k] == probsh_k[0] + probsh_k[1] - 2 * probsh_k[0] * probsh_k[1]);
// 					model.addQConstr(allVars[i][j - 1].Probs[Starthc + k] == probsh_k[0] * probsh_k[1]);
// 				}
// 			}
...
vector<GRBVar> generate_constraints_of_ConnectOrderOpt_last_stage(vector<vector<GRBVar>> probs_last_stage, vector<vector<GRBVar>> connect_last_stage, GRBModel& model){
	vector<GRBVar> Errors(probs_last_stage.size());
	for (int i = 0; i < probs_last_stage.size(); i++)
	{
		int nConnect = (probs_last_stage[i].size() > 2) ? (probs_last_stage[i].size() - 2) : 0;
		GRBLinExpr sumBits = 0;
		GRBQuadExpr sumError = 0;
		for (int j = 0; j < nConnect; j++)
		{
			sumBits += connect_last_stage[i][j];
			sumError += connect_last_stage[i][j] * probs_last_stage[i][j];
		}
		model.addConstr(sumBits == nConnect);
		Errors[i] = model.addVar(0, GRB_INFINITY, 0, GRB_CONTINUOUS, "last_stage_error_" + to_string(i));
		model.addQConstr(Errors[i] == sumError);
	}
	return Errors;
}