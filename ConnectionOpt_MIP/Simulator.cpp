...
void get_name_value(string tmp, int& f, int& h, int& ac32, int& ac42, int& V, ifstream& f0)
{
	stringstream ss;
	//fi_j
	string tmp_s, tmp_v;
...
void ConstructCT(vector<vector<int>>& F, vector<vector<int>>& H, vector<vector<int>>& AC32, 
	vector<vector<int>>& AC42, vector<vector<int>>& V)
{
	ifstream f0;
	string tmp;
	...
	while (getline(f0, tmp))
	{
		if (tmp[0] == 'f' && i < stages_num)
		{
			int f, h, ac32, ac42, v;
			int index;
			get_name_value(tmp, f, h, ac32, ac42, v, f0);
			F[i][j] = f; H[i][j] = h; AC32[i][j] = ac32; AC42[i][j] = ac42;
			V[i][j] = v;
			get_index(tmp, index);
