...
void generate_variables_of_multiplier(vector<vector<GRBVar>>& variables_fa, vector<vector<GRBVar>>& variables_ha,
	vector<vector<GRBVar>>& variables_V, vector<vector<GRBVar>>& variables_ac32,
	vector<vector<GRBVar>>& variables_ac42, GRBModel& model)
{
	for (int i = 0; i < stages_num + 1; i++)
	{
		vector<GRBVar> variables_fa_of_each_stage;
...
void generate_constraints_of_h_f_V(vector<vector<GRBVar>> variables_fa, vector<vector<GRBVar>> variables_ha, vector<vector<GRBVar>> variables_V,
	vector<vector<GRBVar>> variables_ac32, vector<vector<GRBVar>> variables_ac42, GRBModel& model)
{
...
void generate_constraints_error_compensate0(vector<vector<int>> num_ac32, vector<vector<int>> num_ac42,
	vector<vector<GRBVar>> variables_ac32, vector<vector<GRBVar>> variables_ac42, GRBModel& model)
{
	int total_num_ac32 = 0, total_num_ac42 = 0;
	GRBLinExpr grb_num_ac32 = 0, grb_num_ac42 = 0;
	for(int j = 0; j < num_ac32[0].size(); j++) {
				
		for (int i = 0; i < num_ac32.size(); i++)
		{
			total_num_ac32 += num_ac32[i][j];
			total_num_ac42 += num_ac42[i][j];
			grb_num_ac32 += variables_ac32[i][j];
			grb_num_ac42 += variables_ac42[i][j];
		}
		
		
	}

	model.addConstr(grb_num_ac32 <= total_num_ac32);
	model.addConstr(grb_num_ac42 <= total_num_ac42);

	// for(int j = 0; j < num_ac32[0].size(); j++) {
		
	// 	for (int i = 0; i < num_ac32.size(); i++)
	// 	{
	// 		total_num_ac32 += pow(2, num_ac32[0].size() - j - 1) * num_ac32[i][j];
	// 		total_num_ac42 += pow(2, num_ac32[0].size() - j - 1) * num_ac42[i][j];
	// 		grb_num_ac32 += pow(2, num_ac32[0].size() - j - 1) * variables_ac32[i][j];
	// 		grb_num_ac42 += pow(2, num_ac32[0].size() - j - 1) * variables_ac42[i][j];
	// 	}
...
void generate_constraints_error_compensate1(vector<vector<int>> num_ac32, vector<vector<int>> num_ac42,
	vector<vector<GRBVar>> variables_ac32, vector<vector<GRBVar>> variables_ac42, GRBModel& model)
{
...
void generate_constraints_error_compensate2(vector<vector<int>> num_ac32, vector<vector<int>> num_ac42,
	vector<vector<GRBVar>> variables_ac32, vector<vector<GRBVar>> variables_ac42, GRBModel& model)
{
	int total_num_ac32 = 0, total_num_ac42 = 0;
	GRBLinExpr grb_num_ac32 = 0, grb_num_ac42 = 0;
